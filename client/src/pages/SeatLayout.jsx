...
const SeatLayout = () => {
  ...
  const [show]=useState(()=>{
    ...
    return found ? { movie: found, dateTime: dummyDateTimeData } : null
  })
  ...
  const renderSeats=(row,count=9)=>(
    <div className='flex gap-2 mt-2' key={row}>
      ...
      {Array.from({ length: count }, (_, i) => {
       ...
       return(
        <button key={seatId} onClick={()=>handleSeatClick(seatId)} className={`w-8 h-8 rounded border border-primary/60 cursor-pointer ${selectedSeats.includes(seatId)&&'bg-primary text-white'}`}>{seatId}</button>
       );
      })}
      </div>
    </div>
      ...
      <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
        ...
        <div className='flex flex-col items-center mt-10 text-xs text-gray-300'>
         ...
         <button className='flex items-center gap-1 mt-20 px-10 py-3 text-sm
             ...
             active:scale-95' onClick={()=>navigate('/my-bookings')}>Proceed to Checkout
